...
BATCH_SIZE = 256        # batch used for training
...
class AgentsGroup():
    """
    Group of agents for multi agent training
    """
    def __init__(self, no_agents, state_size, action_size, seed):
        """
        Initializes agents group with given params
        @param: no_agents[int]: no of agents used in training
        @param: state_size: state size of the environemnt
        @param: action_size: action space size of the environemt
        """
        # Use common relay buffer for all the agents
        self.memory = ReplayBuffer(action_size, BUFFER_SIZE, BATCH_SIZE, seed)
        self.agents = [Agent(self.memory, state_size, action_size, seed) for _ in range(no_agents)]
  
    def reset_noises(self):
        """
        Reste OU noise for the agents
        """
        _ = [agent.reset() for agent in self.agents]
  
    def step(self, states, actions, rewards, next_states, dones):
        """
        Step agents from the interaction with the environment
        @param: states: current state for all the agents
        @param: actions: action taken in this state for all agents
        @param: rewards: reward received for all agents
        @param: next_states: state output based on action for all agents
        @param: dones: flag to check if termination condition is reached for all agents
        """
    ...
    def act(self, states):
        """
        Agents output for the state
        """
        return [agent.act(state) for agent, state in zip(self.agents, states)]
  
    def get_parameters_to_save(self):
        """
        Returns each agents mode weights to save
        """
        return [{
